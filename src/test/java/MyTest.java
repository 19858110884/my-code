...
public class MyTest {
    ...
    public void addStudent() {
        ...
        ClassInfo classInfo = classInfos.get(classInfos.size() - 1);
            Integer row = studentService.addStudent(new Student("张三", "2019210009", "男", "河南", "200001", classInfo.getClassId()));
        if (row > 0) {
            System.out.println("添加成功");
        } else {
            System.out.println("添加失败");
    ...
    public void queryStudentById2() {
        System.out.println(studentService.queryStudentById("2019210009").getName().equals("张三"));
    }
